...
export default function Customers() {
  ...
  useEffect(() => {
    if (selectedCustomer && view === 'form') {
      setFormData({
        ...selectedCustomer,
        price_per_km: selectedCustomer.price_per_km?.toString() || '',
        base_price: selectedCustomer.base_price?.toString() || '',
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (selectedCustomer) {
        await updateMutation.mutateAsync({ id: selectedCustomer.id, data: dataToSave });
      } else {
        ...
        <form onSubmit={handleSubmit}>
          <Card>
            ...
            <CardContent className="p-6 space-y-6">
              ...
              <div className="space-y-4">
                <h3 className="font-semibold">Preisliste</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                  ...
                  <div>
                    <Label>Preis pro Kilometer (€)</Label>
                    <Input 
                      type="number"
                      step="0.01"
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                {selectedCustomer.base_price && (
                  <div className="p-3 bg-gray-50 rounded-lg">
                    <p className="text-sm text-gray-500">Grundpreis</p>
                    <p className="text-2xl font-bold text-[#1e3a5f]">
                      {selectedCustomer.base_price.toFixed(2)} €
                    </p>
                  </div>
                )}
                {selectedCustomer.price_per_km && (
                  <div className="p-3 bg-gray-50 rounded-lg">
                    <p className="text-sm text-gray-500">Preis pro km</p>
                    <p className="text-2xl font-bold text-[#1e3a5f]">
                      {selectedCustomer.price_per_km.toFixed(2)} €
                    </p>
                  </div>
                )}
                {!selectedCustomer.base_price && !selectedCustomer.price_per_km && (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {customer.price_per_km && (
                      <p className="text-sm text-green-600 font-medium mt-1">
                        {customer.price_per_km.toFixed(2)} € / km
                      </p>
                    )}
                  </div>
                </div>
